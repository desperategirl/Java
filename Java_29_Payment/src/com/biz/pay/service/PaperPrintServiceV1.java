 ...
 *  => payStrList
 * 읽은 정보(리스트)를 한 사람분의 급여를 
 * 각각 PaperMakeService에 주입을 하고
 * 화폐 매수를 계산한 결과를 가져와서 
 * console에 보여줄 것 
 */
public class PaperPrintServiceV1 {
	...
	public PaperPrintServiceV1(String payFile) {
		// 기본생성자를 호출해서
		// pr 객체와 pm 객체를 생성하고 인스턴스를 만들어 둔다
		this(); // 기본생성자를 호출하라
	...
	public void makePaperList() throws IOException {
		...
		// payStrList는 문자열(String)을 담고 있는 List형이기 때문에
		// List 형이기 때문에
		// for 반복문을 이용해서 한 개씩 추출이 된다.
		for (String pay : payStrList) {
			// pay 사번:급여액 형식의 문자열이 들어 있다.
			// pay를 사번, 급여 부분으로 분해
